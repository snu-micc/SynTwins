...
class Syn_REINVENT_Optimizer(BaseOptimizer):
    def __init__(self, syntwins_loader, args=None):
        super().__init__(args)
        self.model_name = "syn_reinvent"
        self.syntwins_loader = syntwins_loader
        
    def get_syntwins(self, smiles_list):
        twins_list = []
        for smi in smiles_list:
            if Chem.MolFromSmiles(smi):
                self.tree = SynTree(self.syntwins_loader, _nBits=256, k=1, max_depth=2)
                twins = self.tree.get_twins(smi)
                twins_list.append(list(twins.keys())[0])
            else:
                twins_list.append(smi)
        return twins_list
    ...
    def _optimize(self, oracle, config):
        ...
        while True:
            ...
            score1 = np.array(self.oracle(smiles))
            
            twins = self.get_syntwins(smiles)
            score2 = np.array(self.oracle(twins))
